...
def _dist2nearest_dual(lock, list1, list2, global_idx, shared_arr, dist_function, mutations=[]):
    ...
    while global_idx.value < list_len:
        ...
        elem_1 = list1[idx]
        if mutations: mut_elem_1 = mutations[idx]
        for idx_j in range(len(list2)):
            _aux = dist_function(elem_1, list2[idx_j], [mut_elem_1, mutations[idx_j]]) if mutations else dist_function(elem_1, list2[idx_j])
            if _aux != 0:
...
def dist2nearest_dual(list1, list2, dist_function, mutations=[]):
    ...
    try:
        for _ in range(n_proc):
            p = mp.Process(target=_dist2nearest_dual,
                        args=(lock, list1, list2, index, shared_array, dist_function, mutations))
            p.start()
            ps.append(p)
...
def _dense_distance_dual(lock, list1, list2, global_idx, shared_arr, dist_function, mutations=[]):
    """Parallelize a general computation of a distance matrix.
...
def dense_dm_dual(list1, list2, dist_function, mutations=[], condensed=False):
...
def _dense_distance(lock, input_list, global_idx, shared_arr, dist_function, mutations=[]):
    """Parallelize a general computation of a distance matrix.
    ...
    while global_idx.value < list_len:
        ...
        for idx_j in range(idx+1, list_len):
            shared_arr[idx, idx_j] = dist_function(elem_1, input_list[idx_j], [mut_elem_1, mutations[idx_j]]) if mutations else dist_function(elem_1, input_list[idx_j])


def dense_dm(input_array, dist_function, mutations=[], condensed=False):
    """Compute in a parallel way a distance matrix for a 1-d array.
    ...
    try:
        for _ in range(n_proc):
            p = mp.Process(target=_dense_distance,
                        args=(lock, input_array, index, shared_array, dist_function, mutations))
            p.start()
            ps.append(p)
...
def distance_matrix_parallel(input_array, dist_function, mutations=[], condensed=False, sparse_mode=False):
    if sparse_mode:
        return sparse_dm(input_array, dist_function, mutations=mutations, condensed=condensed)
    return dense_dm(input_array, dist_function, mutations=mutations, condensed=condensed)
